...
Swiper = function (selector, params, callback) {
  ...
  if (!window.addEventListener) {
    ...
    Element.prototype.addEventListener = HTMLDocument.prototype.addEventListener = addEventListener = function (type, listener, useCapture) {
      this.attachEvent('on' + type, listener);
    };
    Element.prototype.removeEventListener = HTMLDocument.prototype.removeEventListener = removeEventListener = function (type, listener, useCapture) {
  ...
  var wrapper = dQ(selector + ' .' + params.wrapperClass).item(0), isHorizontal, slideSize, numOfSlides, wrapperSize, direction, isScrolling, containerSize;
  ...
  function onTouchEnd(event) {
    ...
    if (params.freeMode) {
      if ((_this.times.end - _this.times.start) < 300 && params.freeModeFluid) {
        ...
        if (isHorizontal)
          _this.setTransform(newPosition, 0, 0); else
